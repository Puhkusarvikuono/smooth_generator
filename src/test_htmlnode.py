...
class TestHTMLNode(unittest.TestCase):
    ...
    def test_parent_to_html_with_no_children(self):
        with self.assertRaises(ValueError):
            childless_node = ParentNode("div", None)

    def test_parent_rejects_non_iterable_value(self):
        child_node = LeafNode("span", "child")
        with self.assertRaises(TypeError):
            ParentNode(tag="div", children=123)
    
    def test_parent_tag_with_props(self):
        parent_node = ParentNode(
            ...
            {"class": "greeting", "id": "main-message"}
            )
        self.assertEqual(
                parent_node.to_html(),
                '<p class="greeting" id="main-message"><b>Hello</b><i>World</i></p>'
            ) 
